            ...
            try:
                ...
                if data: # If socket received any data
                    ...
                    if user_data[0] == "SIGNUP":
                        ...
                        dbprotocol.cursor.execute(f'INSERT INTO {protocol.user_tbl} ("username", "password", "datetime") VALUES (?, ?, ?)',
                    ...
                    if user_data[0] == "LOGIN":
                        # Prevent SQL injection
                        result = dbprotocol.cursor.execute(f'SELECT * FROM {protocol.user_tbl} WHERE username = ? AND password = ?', (user_data[1], user_data[2])).fetchone()
                        if result:
